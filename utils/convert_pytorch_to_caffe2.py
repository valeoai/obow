"""
Converts a torchvision model to caffe2 format.
"""
...
def _rename_basic_resnet_weights(layer_keys):
    # Make Caffe2 compatible - architecture:
    # https://github.com/facebookresearch/fair_self_supervision_benchmark/blob/master/self_supervision_benchmark/modeling/supervised/resnet_supervised_finetune_full.py
...
def _rename_weights_for_resnet(weights):
    ...
    print("Remapping PyTorch weights")
    ...
    for k in original_keys:
        ...
        if 'fc' in k:
            continue
        w = v.data.cpu().numpy()
        print("PyTorch name: {: <{}} mapped name: {}".format(
            k, max_pth_key_size, key_map[k]))
        new_weights[key_map[k]] = w
    print('Number of blobs: {}'.format(len(new_weights)))
    return new_weights
...
def convert_rgb2bgr(state_dict):
    key = 'conv1.weight'
    weight = state_dict[key]
    assert (weight.shape == (64, 3, 7, 7))
    weight_np = weight.detach().numpy()
    weight_np = weight_np[:, ::-1, :, :].copy()
    weight = torch.from_numpy(weight_np)
    state_dict[key] = weight
    print(f'BGR ===> RGB for {key}.')
    return state_dict
    ...
    try:
        ...
        try:
            ...
            if getattr(e, 'errno', None) != errno.ENOENT:  # We expect ENOENT
                print("Could not delete temp file %r",
...
def main():
    ...
    parser.add_argument('--rgb2bgr', dest='rgb2bgr', default=False,
                        help='Revert bgr order to rgb order')
    args = parser.parse_args()

    # load the pytorch model first
    state_dict = _load_pytorch_weights(args.pth_model)
    ...
    if args.rgb2bgr:
        state_dict = convert_rgb2bgr(state_dict)

    blobs = _rename_weights_for_resnet(state_dict)
    blobs = dict(blobs=blobs)
    print('Saving converted weights to: {}'.format(args.output_model))
